...
# y = max(0, x)
def ReLULayer(x,dim1,dim2,dropout_prob):
    import tensorflow as tf
    W_h = tf.Variable(tf.random_normal([dim1,dim2], stddev=0.1))
...
def Model(x,dim1,dim2, n_hidden):
    ...
    dimH1, dimH2, dimH3=n_hidden, n_hidden, n_hidden
    
    #hidden layer 1:
    h1=ReLULayer(x,dim1,dimH1,dropout_prob=0.8)
    #hidden layer 2:
    h2=ReLULayer(h1,dimH1,dimH2,dropout_prob=0.8)
    #hidden layer 3:
    h3=ReLULayer(h2,dimH2,dimH3,dropout_prob=0.8)
    
    # output layer (Sigmoid layer):
    # Sigmoid layer doesm't change dimension of data.
    logits=LinearLayer(h3,dimH3,dim2)
